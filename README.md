...
국가·지방공무원 인사 업무 담당자를 위한 전용 MCP 서버(`src/hr_main.py`)가 별도로 제공됩니다.
인사 질문을 하면 AI가 관련 법령·인사혁신처 예규·지침을 검토해 **담당자 할 일을 시계열순으로 정리**하고,
**규정 요약과 law.go.kr 원문 링크**를 함께 제공합니다.

- 인사 이벤트 11종(채용·승진·전보·파견·휴직·징계·퇴직·평정·보수·겸직) 시계열 체크리스트 내장
- 인사혁신처 법령·훈령·예규·고시 목록 검색 + 행정규칙 본문 원문 조회
- Claude Desktop(STDIO) 및 Claude 웹/모바일 커넥터(HTTP) 지원

👉 설치·사용법: [docs/tax-tribunal-hr/README.md](docs/tax-tribunal-hr/README.md)
